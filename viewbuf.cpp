...
void UI_Mainwindow::setup_viewbuf()
{
  ...
  if(hasprefilter)
  {
    ...
    if(viewbuf==NULL)
    {
      ...
      printf("Internal error: Memory allocation error:\n\"prefilterbuf\"\n");
      //remove_all_signals();
	  exit(7);
    }
    ...
    for(i=0; i<signalcomps; i++)
    {
      if(!i)
      {
        ...
        if((signalcomp[i]->viewbufsize>0)&&(datarecords<signalcomp[i]->edfhdr->datarecords))
        {
          ...
          {
            live_stream_active = 0;
            printf("A read error occurred. 2\n");
			exit(7);
          }
        }
      }
      else
      {
        ...
        if(!skip)
        {
          ...
          if((signalcomp[i]->viewbufsize>0)&&(datarecords<signalcomp[i]->edfhdr->datarecords))
          {
            ...
            {
              live_stream_active = 0;
              printf("A read error occurred. 3\n");
			  exit(7);
            }
          }
        }
...
*/
    ...
    for(i=0; i<signalcomps; i++)
    {
      if(signalcomp[i]->ecg_filter == NULL) continue;
...
/*
  ...
  if(totalsize)
  {
    ...
    {
      live_stream_active = 0;
      printf("The system was not able to provide enough resources (memory) to perform the requested action. Decrease the timescale and try again.\n");
	  exit(7);
    }
  }
  ...
  for(i=0; i<signalcomps; i++)
  {
    if(!i)
    {
      ...
      else
      {
        ...
        {
          live_stream_active = 0;
			 printf("A read error occurred. 5");
			 exit(7);
        }
      }
    }
    else
    {
      ...
      else
      {
        ...
        if(!skip)
        {
          ...
          {
            live_stream_active = 0;
            printf("A read error occurred. 6");
			exit(7);
          }
        }
      }
