...
UI_JumpMenuDialog::UI_JumpMenuDialog(UI_Mainwindow *w_parent)
{
  mainwindow = w_parent;
...
void UI_JumpMenuDialog::offsettime_changed()
{
  long long milliseconds;
  ...
  long hour = 0, minute = 0, second = 6, msec = 0;

  milliseconds = hour * 3600000LL;
  milliseconds += minute * 60000LL;
  milliseconds += second * 1000LL;
  milliseconds += msec;
...
void UI_JumpMenuDialog::jumpbutton_pressed()
{
  ...
  long hour = 0, minute = 0, second = 1, msec = 0;
  ...
  //milliseconds += ((long long)daybox1->value() * 86400000LL);
  printf("mainwindow->viewtime_sync = %d\n", mainwindow->viewtime_sync);
  ...
  if(mainwindow->viewtime_sync==VIEWTIME_SYNCED_ABSOLUT)
  {
    ...
    for(i=0; i<mainwindow->files_open; i++)
    {
      if(i!=mainwindow->sel_viewtime)
      {
		...
		printf("mainwindow->edfheaderlist[%d]->viewtime = %ld\n", i, mainwindow->edfheaderlist[i]->viewtime);
      }
    }
  }
